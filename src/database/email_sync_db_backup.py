...
class EmailSyncDatabase:
    ...
    def _initialize_database(self):
        ...
        if self.use_pool:
            ...
            with self._pool.get_connection() as conn:
                self.conn = conn  # 临时设置用于创建表
                self._create_tables()
            self.conn = None  # 清除直接连接
        else:
            # 直接连接
            ...
            self.conn.execute("PRAGMA busy_timeout=30000")  # 30秒忙等待
    ...
    def search_emails(self, query: str, account_id: str = None, 
        ...
        if self.use_pool:
            results = self._execute(sql, params, commit=False)
        else:
    ...
    def _get_connection(self):
        ...
        if self.use_pool:
            ...
            return self._dummy_context_manager(self.conn)
    
    @staticmethod
    def _dummy_context_manager(conn):
        """为直接连接创建上下文管理器"""
    ...
    def _execute(self, query: str, params: tuple = (), commit: bool = True):
        """执行SQL查询"""
        if self.use_pool:
            return self._pool.execute(query, params, commit)
        else:
